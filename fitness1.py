...
# Create tabs
tab1, tab2, tab3 = st.tabs(["Speed Score Calculator", "BMI Calculator", "Burst Score Calculator"])
...
with tab3:
    st.header("Burst Score Calculator")
    ...
    if st.button("Calculate Burst Score", key="burst_score_button"):
        burst_score = (vertical_jump * 1.7) + (broad_jump * 0.5)
        st.success(f"Your Burst Score is: **{burst_score:.2f}**")

st.write("The formula for Burst Score is vertical jump * 1.7 + broad jump * 0.5")
    # Explanation of the formula
st.markdown("""
